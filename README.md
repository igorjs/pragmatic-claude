...
- `/implement`: execute an approved `/scope` plan or `/adr` blueprint. Execute-only (it won't design; run `/scope` or `/adr` first); runs on Sonnet, delegating edits to subagents and reviewing each via TDD, committing each unit. `--auto` runs Work Units autonomously and opens a PR.
