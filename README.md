...
- `/implement`: execute an approved `/scope` plan or `/adr` blueprint. Execute-only (it won't design; run `/scope` or `/adr` first); runs on Sonnet, delegating edits to subagents and reviewing each via TDD. It runs Work Units in dependency order, dispatches parallel-safe ones to concurrent agents, and commits each as a small commit with `/commit-and-push -y`. With no arguments it lists saved plans to pick from. `--auto` runs Work Units autonomously and opens a PR.
