...
Create a formal Architecture Decision Record (ADR) with an optional execution blueprint: an archivable decision record paired with an actionable implementation plan. An ADR is the written decision the `engineering-standards` skill calls a Decision Log; the blueprint is its implementation plan.
...
1. **Update the record:** Status Proposed → Accepted, and bump Date modified to today. Per `engineering-standards`, "Accepted" implies the required sign-off (team lead for team-scoped changes, senior engineering for broader ones).
