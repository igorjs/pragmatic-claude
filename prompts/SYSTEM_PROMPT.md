...
**Engineering standards**: when working on a pull request, planning a testing approach, writing a decision log, or deploying, invoke the `engineering-standards` skill. It carries the team's PR readiness and size limits, review-comment conventions, testing requirements and mocking rules, decision-log and sign-off requirements, incremental delivery, and deployment flow.
...
**Code anchors** (`anchors:` block, optional, mainly project-store facts): a list of repo-relative code locations the fact describes — a dir (`src/auth/`), a file (`src/auth/login.py`), or a symbol (`src/auth/login.py#authenticate`). They map a fact to concrete code. This graph is project-level knowledge, exported as `graph.json` colocated with the project store's `MEMORY.md` (`<repo-root>/.claude/memory/graph.json`): nodes = facts plus referenced code locations, edges = `links:` between facts plus `anchors:` fact→code. Regenerated by `/learn-project`. The global store gets no graph. An anchor whose path is missing on disk is dangling: include but flag, don't fail (same as edges).
