...
description: Use when working on pull requests, planning testing approach, writing decision logs, or thinking about deployment. Covers team engineering process: PR readiness criteria, size limits, review comment conventions, testing requirements (unit/integration/E2E), mocking guidelines, decision log requirements, incremental delivery, and deployment flow.
---
...
- Significant changes SHOULD have a written Decision Log before implementation begins.
- A Decision Log MUST describe the problem, options considered, the chosen approach, and trade-offs.
- Changes handled within the team (API endpoints, service interfaces, schema changes) require team lead sign-off.
- Broader changes (architectural, cost-impactful infrastructure, new technologies or patterns) require senior engineering sign-off.
- A Decision Log MUST be signed off before implementation begins.
